...
class UserAuthClass(TestSetUp):
    ...
    def test_missing_first_or_last_name(self):
        ...
        response = self.app.post("/api/v2/auth/register",
                                 data=json.dumps(dict(username="testusername",
                                                      password="testpassword", first_name="", last_name="last")),
                                 content_type="application/json")
        self.assertEqual(response.status_code, 400)
        response_msg = json.loads(response.data.decode("UTF-8"))
        self.assertIn("required", response_msg["Message"])
    ...
    def test_get_one_user(self):
        ...
        response = self.app.get('/api/v2/auth/users/{}'.format(User.query.order_by(User.created_at).first().id),
                                content_type="application/json",
    ...
    def test_read_user_business(self):
        ...
        response = self.app.get('/api/v2/auth/users/{}/businesses'.format(User.query.order_by(User.created_at).first().id),
                                content_type="application/json")
        self.assertEqual(response.status_code, 200)
